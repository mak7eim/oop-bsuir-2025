...
func (dl *DataLoader) parseTransport(record []string) (domain.Transport, error) {
	...
	name := transport.Name
	switch {
	case strings.Contains(name, "Грузовик") || strings.Contains(name, "Поезд"):
		transport.Type = domain.Land
	case strings.Contains(name, "Танкер"):
		transport.Type = domain.Water
	case strings.Contains(name, "Самолет") || strings.Contains(name, "Вертолет"):
...
func (dl *DataLoader) parseCargo(record []string) (string, CargoInfo, error) {
	...
	cost, err := strconv.ParseFloat(strings.Replace(record[3], ",", ".", -1), 64)
	if err != nil {
		return "", CargoInfo{}, fmt.Errorf("ошибка парсинга стоимости: %w", err)
	}
